...
void CPicture::Save(const std::wstring &filename)
{
	...
	//root->SetAttribute(L"TreeSeed",  mTree1->GetSeed());
	//root->SetAttribute(L"StartFrame", mTree1->GetStart());
	//root->SetAttribute(L"TreeTwo", GetTreeTwo());
...
void CPicture::Load(const std::wstring &filename)
{
    ...
    try
    {
		...
		//mTree1->SetStartFrame(root->GetAttributeIntValue())
